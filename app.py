...
def process_video(input_path, output_path):
    ...
    # Configure the modern Landmarker instance
    ...
    with PoseLandmarker.create_from_options(options) as landmarker:
        while cap.isOpened():
            ...
            # Convert frame to MediaPipe Image object format
            mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            
            # Calculate running timestamp in milliseconds
            frame_timestamp_ms = int(cap.get(cv2.CAP_PROP_POS_MSEC))
            
            # Run detection
            detection_result = landmarker.detect_for_video(mp_image, frame_timestamp_ms)
            
            # Draw overlay skeleton points manually if any skeletons are found
            if detection_result.pose_landmarks:
                for landmarks in detection_result.pose_landmarks:
                    for lm in landmarks:
                        cx, cy = int(lm.x * width), int(lm.y * height)
                        # Draw a small circle for every tracking joint detected
